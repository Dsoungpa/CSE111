   ...
   if(words.size() == 2){
      ...
      if(words[1] == "/"){
         ...
         it != cD->getContent()->getDirent().end(); it++){
            stringstream ss;
            ss << it->second->getContent()->getAm();
            cout << "      " << it->second->get_inode_nr() << setw(8) << right
            << ss.str() << "  " << it->first;
            if(state.getCwd()->getContent()->getPtr(it->first)->getContent()
            ->getError() != "plain file"){
               cout << "/";
            }
            cout << endl;
...
void fn_make (inode_state& state, const wordvec& words) {
   ...
   if(state.getCwd()->getContent()->getPtr(filename) == nullptr){
      ...
      newFile->getContent()->setAm(contents_size);
   }
   else{
